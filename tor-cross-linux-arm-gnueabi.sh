# Tor Static Cross compile From Linux64 to Linux-ARM-32Bit
...
# Build libevent:
    ...
    ./configure --prefix="/libevent-2.1.11-stable/install" --enable-static --disable-shared --host=arm-linux-gnueabi LDFLAGS="-static"
    make
    make install
    cd ..
...
# Build Tor:
    ...
    ./configure --enable-static-tor --host=arm-linux-gnueabi --prefix="$HOME/tor/install" --with-libevent-dir="/libevent-2.1.11-stable/install/" --with-openssl-dir="/openssl-1.0.2j/install/" --with-zlib-dir="/zlib-1.2.11" --disable-tool-name-check
